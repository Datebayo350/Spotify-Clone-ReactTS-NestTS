...
const AlbumCard = (item: any, layout: DataViewLayoutType) => {
  ...
  const header = <Image src={getImage(item)} alt={`Image ${item.name}`} width="100%"></Image>;
  ...
  const date = <span className="date">{item.date}</span>;

  const subTitle = (
    <div className="flex align-items-center">
      <span className="">{releaseDateYear}</span>
      <i className="pi pi-circle-fill custom-icon m-1"></i>
      <span className="">{artist}</span>
    </div>
  );

  return <Card title={item.name} header={header} subTitle={subTitle} className="text-sm"></Card>;
};
